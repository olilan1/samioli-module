...
export function isAlly(
    targetToken: TokenPF2e,
    casterActor: ActorPF2e,
    casterToken?: TokenPF2e
): boolean {
    ...
    const disposition = casterToken?.document.disposition ??
        casterActor.getActiveTokens()[0]?.document.disposition;
    return targetToken.document.disposition === disposition;
}

/**
 * Checks if an actor is conscious and alive based on their HP and conditions.
 */
export function isConsciousAndAlive(
    actor: ActorPF2e,
    options: {
    ...
    const deadCondition = options.deadCondition ?? "dead";
    const unconsciousCondition = options.unconsciousCondition ?? "unconscious";
    const minHp = options.minHp ?? 0;

    const hp = actor.system.attributes.hp?.value ?? 0;
    const isDead = actor.conditions.has(deadCondition) || hp <= minHp;
    const isUnconscious = actor.conditions.has(unconsciousCondition);
    return !isDead && !isUnconscious;
}

/**
 * Checks if there is a line of effect between two tokens or points by testing for collision.
 * 
 * @param origin - The starting token or point coordinates.
 * @param target - The target token or point coordinates.
 * @param options.collisionType - The backend type to check collision on.
 *        Valid values: "move" (walls that block movement), "sight" (walls that block sight),
 *        "light" (walls that block light), "sound" (walls that block sound). Defaults to "move".
 * @param options.type - The collision type parameter passed to the backend. Defaults to "move".
 * @param options.mode - Collision search mode.
 *        Valid values: "any" (returns true on first collision found, fast),
 *        "all" (collects all collisions), "closest" (finds closest collision). Defaults to "any".
 */
export function hasLineOfEffect(
    origin: Point | TokenPF2e,
    target: Point | TokenPF2e,
    options: {
    ...
    const originPoint = "center" in origin ? origin.center : origin;
    const targetPoint = "center" in target ? target.center : target;

    const collisionType = options.collisionType ?? "move";
    const type = options.type ?? "move";
    const mode = options.mode ?? "any";

    // @ts-expect-error dynamic indexing on polygonBackends is valid
    const backend = CONFIG.Canvas.polygonBackends[collisionType];
...
/**
 * Calculates the distance in feet between two tokens, points, or a mix of both.
 */
export function getDistance(
    origin: TokenPF2e | Point,
    target: TokenPF2e | Point
): number {
    if ("distanceTo" in origin && "distanceTo" in target) {
    ...
    const path = [originPoint, targetPoint];
    return Math.round(canvas.grid.measurePath(path).euclidean);
}

/**
 * Checks if the distance between two tokens or points is within a specified number of feet.
 */
export function isWithinDistance(
    ...
    return getDistance(origin, target) <= maxDistanceFeet;
}
