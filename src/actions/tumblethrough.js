...
export async function startTumbleThrough(ChatMessagePF2e) {
    ...
    || ChatMessagePF2e.flags.pf2e.context.outcome === "success") {
        ...
        await animateSuccessfulTumble(token, targetTokens, rotationValue, x, y);
    ...
    || ChatMessagePF2e.flags.pf2e.context.outcome === "failure") {
        ...
        await animateFailureTumble(x, y, token, targetTokens, rotationValue, originalTokenPositionX, OriginalTokenPositionY);
...
async function animateFailureTumble(x, y, token, targetTokens, rotationValue, originalTokenPositionX, OriginalTokenPositionY) {
    const fallDownX = (x / 2);
    const fallDownY = (y / 2);
...
async function animateSuccessfulTumble(token, targetTokens, rotationValue, x, y) {
