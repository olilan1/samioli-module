export function checkForBravado(chatMessage) {
  ...
  export async function checkForExtravagantParryOrElegantBuckler(chatMessage) {
    const { flags } = chatMessage;
    const swashbuckler = game.actors.get((flags?.pf2e?.context?.target?.actor).split('.').pop());
    ...
    const hasElegantBuckler = async (swashbucker) => (await swashbucker.items.contents).some(item => item.type === "feat" && item.system.category === "class" && item.system.slug === "elegant-buckler");
    const isAttackRoll = flags?.pf2e?.context?.type === "attack-roll";
    const isFailure = flags?.pf2e?.context?.outcome === "failure"
    const isCriticalFailure = flags?.pf2e?.context?.outcome === "criticalFailure"
  
    if ((hasDuelingParry && isAttackRoll && (isFailure || isCriticalFailure))
      || hasElegantBuckler && hasShieldRaised && isAttackRoll && isCriticalFailure) {
      applyPanache(swashbuckler, "failure")
    }
  }